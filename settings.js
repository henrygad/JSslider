...
const Settings = (arrSlides = 'none', {
    ...
    return( //display slides in the DOM
        `<div class='${slidesWrapperClass}' id='${slidesWrapperId}' style="position: relative; top: 0%; left: 0%; display: flex; flex-direction: ${slidemove_from == 'top' || slidemove_from == 'bottom'? 'column': 'row'}; width: ${hrWidthOrVrheight === 'left'? slidesWrapperHrWidth: 100}%; height: ${hrWidthOrVrheight === 'top'? slidesWrapperVrHeight: 100}%;">
             ${
                createNewArrslides.map(value=>(
                     value.join(' ')
                )).join(' ')
            }
            ${createNewArrslides[0].join(' ')}      
        </div>`
    )
}
